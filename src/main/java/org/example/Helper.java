...
public class Helper {
    ...
    //Ordering scheme as such ID Move > TT move > MVV-LVA > Promotion > Killer 1 > Killer 2 > checks
    private int calculateMoveValue(Board board, Move move, TranspositionTable transpositionTable, int ply){
        ...
        if(isCapture(board,move)){
            return 3000 + MVV_LVA(board,move);
        }

        // Promotion Handling
        if (move.getPromotion() == Piece.WHITE_QUEEN || move.getPromotion() == Piece.BLACK_QUEEN) {
            return 2000; // Promotions are highly prioritized
        }
        //Killer moves
